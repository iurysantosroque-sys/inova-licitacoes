...
function renderDocumentControlWorkspace(){
  ...
  el.innerHTML=`<div class="document-control-grid">${eligible.map(tender=>`<button type="button" class="document-control-folder" data-control-tender="${esc(tender.id)}"><img class="document-folder-icon" src="assets/controle-documentos.png" alt=""><span><strong>${esc(tender.numero||'Edital')}</strong><small>${esc(tender.orgao||tender.cidade||'Órgão não informado')}</small><em>${esc(tender.cidade||'')}</em></span><span class="document-folder-arrow">›</span></button>`).join('')}</div><div class="document-control-note">As pastas são vinculadas ao edital e ficam prontas para receber as subpastas que você definir.</div><div id="documentControlFolderModal" class="document-control-folder-modal" hidden><div class="document-control-folder-modal-card"><div class="document-control-folder-modal-head"><div><span class="eyebrow">Pasta da licitação</span><h3 id="documentControlFolderTitle"></h3><p id="documentControlFolderSubtitle"></p></div><button type="button" id="documentControlFolderClose" aria-label="Fechar pasta">×</button></div><div class="document-control-folder-empty"><img src="assets/controle-documentos.png" alt=""><strong>Pasta pronta para organização</strong><span>As subpastas e documentos desta licitação serão adicionados aqui.</span></div><button type="button" class="document-control-folder-back" id="documentControlFolderBack">← Voltar para as licitações</button></div></div>`;
