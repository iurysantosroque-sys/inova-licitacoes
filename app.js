...
function renderAll(){
  ...
  const supplierList=state.fornecedores.length
    ? table(['Fornecedor','Nome fantasia','CNPJ','UF','Vendedor','WhatsApp','E-mail',''],state.fornecedores.map(f=>{
      ...
      return [esc(f.nome),esc(f.nome_fantasia||'-'),esc(f.cnpj||'-'),esc(f.uf||'-'),esc(f.vendedor||'-'),phone?`<span class="supplier-phone">${esc(supplierPhoneLabel(phone))}</span>`:'<span class="muted">Não informado</span>',f.email?`<a class="supplier-email" href="mailto:${esc(f.email)}">${esc(f.email)}</a>`:'<span class="muted">Não informado</span>',`<div class="supplier-actions">${whatsapp?`<a class="action-btn supplier-whatsapp" href="${esc(whatsapp)}" target="_blank" rel="noopener noreferrer">WhatsApp</a>`:'<span class="supplier-no-contact">Sem WhatsApp</span>'}<button class="action-btn" data-edit-supplier="${esc(f.id)}">Editar</button><button class="action-btn danger-btn" data-delete="fornecedor" data-id="${esc(f.id)}">Excluir</button></div>`];
    }))
...
$('#fornecedorForm').addEventListener('submit',async e=>{
  ...
  const f=Object.fromEntries(new FormData(e.target));
  const values={name:f.nome,trade_name:f.nome_fantasia||null,cnpj:f.cnpj||null,state_uf:f.uf||null,contact_name:f.vendedor||null,phone:f.telefone||null,email:f.email||null};
  if(state.demo){const item=state.fornecedores.find(x=>String(x.id)===String(editingSupplierId));if(item)Object.assign(item,{nome:f.nome,nome_fantasia:f.nome_fantasia||'',cnpj:f.cnpj||'',uf:f.uf||'',vendedor:f.vendedor||'',telefone:f.telefone||'',email:f.email||''});}
  ...
  editingSupplierId='';e.target.reset();e.target.hidden=true;$('#novoFornecedor').textContent='＋ Novo fornecedor';renderAll();toast('Fornecedor atualizado.');
},true);
document.addEventListener('click',e=>{const button=e.target.closest('[data-edit-supplier]');if(!button)return;const item=state.fornecedores.find(x=>String(x.id)===String(button.dataset.editSupplier));if(!item)return;editingSupplierId=item.id;const form=$('#fornecedorForm');form.hidden=false;for(const [name,value] of Object.entries({nome:item.nome,nome_fantasia:item.nome_fantasia,cnpj:item.cnpj,uf:item.uf,vendedor:item.vendedor,telefone:item.telefone,email:item.email})){const field=form.elements.namedItem(name);if(field)field.value=value||'';}$('#novoFornecedor').textContent='× Fechar cadastro';form.elements.namedItem('nome')?.focus();});
$('#fornecedorForm').addEventListener('submit',async e=>{e.preventDefault();const f=Object.fromEntries(new FormData(e.target));if(state.demo){state.fornecedores.push({id:crypto.randomUUID(),nome:f.nome,nome_fantasia:f.nome_fantasia||'',uf:f.uf||'',cnpj:f.cnpj||'',vendedor:f.vendedor||'',telefone:f.telefone||'',email:f.email||'',frete_padrao:0,pedido_minimo:0,prazo_dias:null});e.target.reset();renderAll();return toast('Fornecedor adicionado à demonstração.');}const {error}=await supabase.from('suppliers').insert({company_id:currentCompanyId(),name:f.nome,trade_name:f.nome_fantasia||null,cnpj:f.cnpj||null,state_uf:f.uf||null,contact_name:f.vendedor||null,phone:f.telefone||null,email:f.email||null});if(error)return toast(error.message,'error');e.target.reset();toast('Fornecedor cadastrado.');await refreshAll();});
