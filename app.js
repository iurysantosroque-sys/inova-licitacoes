...
async function exportQuotePdf(){
  ...
  const {PDFDocument,rgb,StandardFonts}=window.PDFLib;const base=await fetch('assets/papel-timbrado.pdf').then(r=>r.arrayBuffer());const pdf=await PDFDocument.load(base);let page=pdf.getPages()[0];const font=await pdf.embedFont(StandardFonts.Helvetica);const ink=rgb(.12,.12,.12),line=rgb(.72,.72,.72);const x0=60,x1=390,x2=455,x3=535,rowFont=8.5,rowGap=11;
  ...
  let y=680;page.drawText('TABELA DE COTAÇÃO',{x:x0,y,size:16,font,color:ink});page.drawText(`${tender.orgao||'Órgão comprador'} - ${tender.cidade||''}`,{x:x0,y:y-24,size:10,font,color:ink});y-=58;
  const drawHeader=()=>{const top=y;page.drawRectangle({x:x0,y:top-22,width:x3-x0,height:22,borderColor:line,borderWidth:.7,color:rgb(.96,.96,.96)});[['ITEM',x0+8],['UNIDADE',x1+8],['QUANTIDADE',x2+8]].forEach(([text,x])=>page.drawText(text,{x,y:top-15,size:8.5,font,color:ink}));page.drawLine({start:{x:x1,y:top-22},end:{x:x1,y:top},thickness:.7,color:line});page.drawLine({start:{x:x2,y:top-22},end:{x:x2,y:top},thickness:.7,color:line});y=top-22;};
  drawHeader();items.forEach((item,index)=>{const lines=wrap(`${index+1}. ${item.descricao||''}`,x1-x0-16);const height=Math.max(24,lines.length*rowGap+8);if(y-height<100){page=pdf.addPage([595.5,842.25]);y=760;drawHeader();}page.drawRectangle({x:x0,y:y-height,width:x3-x0,height, borderColor:line,borderWidth:.7});page.drawLine({start:{x:x1,y:y-height},end:{x:x1,y},thickness:.7,color:line});page.drawLine({start:{x:x2,y:y-height},end:{x:x2,y},thickness:.7,color:line});lines.forEach((text,lineIndex)=>page.drawText(text,{x:x0+8,y:y-13-lineIndex*rowGap,size:rowFont,font,color:ink}));page.drawText(String(item.unidade||'-'),{x:x1+8,y:y-14,size:rowFont,font,color:ink});page.drawText(String(item.quantidade??'-'),{x:x2+8,y:y-14,size:rowFont,font,color:ink});y-=height;});const bytes=await pdf.save();const blob=new Blob([bytes],{type:'application/pdf'});const link=document.createElement('a');link.href=URL.createObjectURL(blob);link.download=`cotacao-${tender.numero||'edital'}.pdf`;link.click();setTimeout(()=>URL.revokeObjectURL(link.href),1000);}
