...
function openSupplierProducts(supplier){
  ...
  overlay.innerHTML=`<section class="supplier-products-page" role="dialog" aria-modal="true" aria-labelledby="supplierProductsTitle"><div class="supplier-products-head"><div><span class="eyebrow">Produtos do fornecedor</span><h2 id="supplierProductsTitle">${esc(supplier?.nome||'Fornecedor')}</h2><p>Cadastre e consulte os produtos associados a este fornecedor.</p></div><button type="button" class="supplier-products-close" aria-label="Fechar produtos">×</button></div><div class="supplier-products-empty"><span class="supplier-products-empty-icon">▦</span><strong>Nenhum produto cadastrado</strong><small>Os produtos deste fornecedor aparecerão aqui.</small></div><button type="button" class="supplier-products-back">← Voltar para fornecedores</button></section>`;
  document.body.appendChild(overlay);
  const close=()=>overlay.remove();overlay.querySelector('.supplier-products-close')?.addEventListener('click',close);overlay.querySelector('.supplier-products-back')?.addEventListener('click',close);overlay.addEventListener('click',event=>{if(event.target===overlay)close();});
}
...
function openSupplierProductModal(){const modal=$('#supplierProductModal');if(!modal)return;const select=$('#supplierProductSupplier');if(select)select.innerHTML='<option value="">Selecione o fornecedor</option>'+state.fornecedores.map(f=>`<option value="${esc(f.id)}">${esc(f.nome)}</option>`).join('');$('#supplierProductFile').value='';$('#supplierProductStatus').hidden=true;document.querySelectorAll('[data-supplier-product-tab]').forEach((tab,index)=>tab.classList.toggle('active',index===0));document.querySelectorAll('[data-supplier-product-panel]').forEach((panel,index)=>panel.hidden=index!==0);modal.hidden=false;select?.focus();}
...
$('#saveSupplierProductButton')?.addEventListener('click',()=>{const supplier=$('#supplierProductSupplier')?.value;const file=$('#supplierProductFile')?.files?.[0];const status=$('#supplierProductStatus');if(!supplier)return toast('Selecione um fornecedor.','error');if(!file)return toast('Selecione um arquivo de produtos.','error');if(status){status.hidden=false;status.classList.remove('is-error');status.textContent=`Arquivo "${file.name}" pronto para ser vinculado ao fornecedor selecionado.`;}toast('Importação preparada. O cadastro detalhado de produtos será concluído na próxima etapa.');});
