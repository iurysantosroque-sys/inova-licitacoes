...
async function refreshAll(){
  ...
  state.fornecedores=(suppliers.data||[]).map(f=>({id:f.id,nome:f.name,cnpj:f.cnpj,vendedor:f.contact_name||'',telefone:f.phone||'',email:f.email||'',frete_padrao:Number(f.default_freight_amount||0),pedido_minimo:Number(f.minimum_order||0),prazo_dias:f.delivery_days,raw:f}));
...
function renderAll(){
  ...
  const supplierList=state.fornecedores.length
    ? table(['Fornecedor','CNPJ','Vendedor','WhatsApp','E-mail',''],state.fornecedores.map(f=>{
      const phone=supplierPhone(f); const whatsapp=supplierWhatsappUrl(phone);
      return [esc(f.nome),esc(f.cnpj||'-'),esc(f.vendedor||'-'),phone?`<span class="supplier-phone">${esc(supplierPhoneLabel(phone))}</span>`:'<span class="muted">Não informado</span>',f.email?`<a class="supplier-email" href="mailto:${esc(f.email)}">${esc(f.email)}</a>`:'<span class="muted">Não informado</span>',`<div class="supplier-actions">${whatsapp?`<a class="action-btn supplier-whatsapp" href="${esc(whatsapp)}" target="_blank" rel="noopener noreferrer">WhatsApp</a>`:'<span class="supplier-no-contact">Sem WhatsApp</span>'}<button class="action-btn danger-btn" data-delete="fornecedor" data-id="${esc(f.id)}">Excluir</button></div>`];
    }))
...
$('#fornecedorForm').addEventListener('submit',async e=>{e.preventDefault();const f=Object.fromEntries(new FormData(e.target));if(state.demo){state.fornecedores.push({id:crypto.randomUUID(),nome:f.nome,cnpj:f.cnpj||'',vendedor:f.vendedor||'',telefone:f.telefone||'',email:f.email||'',frete_padrao:0,pedido_minimo:0,prazo_dias:null});e.target.reset();renderAll();return toast('Fornecedor adicionado à demonstração.');}const {error}=await supabase.from('suppliers').insert({company_id:currentCompanyId(),name:f.nome,cnpj:f.cnpj||null,contact_name:f.vendedor||null,phone:f.telefone||null,email:f.email||null});if(error)return toast(error.message,'error');e.target.reset();toast('Fornecedor cadastrado.');await refreshAll();});
