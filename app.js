...
function renderFinance(){
  ...
  tenders.innerHTML=rows.length?table(['Licitação / órgão comprador','Cidade / UF','Data','Balanço líquido',''],rows.map(r=>[`${esc(r.t.numero)}<small class="finance-tender-meta">${esc(r.t.orgao||'Órgão não informado')}</small>`,esc(r.t.cidade||'Cidade não informada'),r.date?dateBR(r.date,false):'-',`<strong class="${r.profit<0?'negative':'positive'}">${money(r.profit)}</strong>`,`<button class="action-btn" data-finance-tender="${esc(r.t.id)}">Ver resultado</button>`])):'<p class="hint">Nenhuma licitação encontrada no período.</p>';
