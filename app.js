...
function renderDocumentControlWorkspace(){
  ...
  el.innerHTML=`<div class="document-control-grid">${eligible.map(tender=>`<button type="button" class="document-control-folder" data-control-tender="${esc(tender.id)}"><img class="document-folder-icon" src="assets/controle-documentos.png" alt=""><span><strong>${esc(tender.numero||'Edital')}</strong><small>${esc(tender.orgao||tender.cidade||'Órgão não informado')}</small><em>${esc(tender.cidade||'')}</em></span><span class="document-folder-arrow">›</span></button>`).join('')}</div><div class="document-control-note">As pastas são vinculadas ao edital e ficam prontas para receber as subpastas que você definir.</div><div id="documentControlFolderModal" class="document-control-folder-modal" hidden><div class="document-control-folder-modal-card"><div class="document-control-folder-modal-head"><div><span class="eyebrow">Pasta da licitação</span><h3 id="documentControlFolderTitle"></h3><p id="documentControlFolderSubtitle"></p></div><button type="button" id="documentControlFolderClose" aria-label="Fechar pasta">×</button></div><div class="document-subfolder-grid"><button type="button" class="document-subfolder"><img src="assets/controle-documentos.png" alt=""><span>Habilitação Jurídica</span></button><button type="button" class="document-subfolder"><img src="assets/controle-documentos.png" alt=""><span>Declarações</span></button></div><div class="document-control-folder-empty"><strong>Selecione uma subpasta para continuar</strong><span>Os documentos e novos tipos de pasta serão adicionados nesta área.</span></div><button type="button" class="document-control-folder-back" id="documentControlFolderBack">← Voltar para as licitações</button></div></div>`;
