...
function renderControlDeclarationFiles(area,files){
  area.innerHTML=`<div class="document-control-generated-head"><strong>Declarações geradas</strong><span>${files.length} arquivos prontos para baixar</span></div><div class="document-control-generated-list">${files.map(file=>`<a href="${file.url}" download="${esc(file.name)}">⇩ ${esc(file.name)}</a>`).join('')}</div>`;
}
