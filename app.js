...
function renderFinance(){
  ...
  tenders.innerHTML=rows.length?table(['Órgão comprador','Cidade / UF','Data','Balanço líquido',''],rows.map(r=>[esc(r.t.orgao||'Órgão não informado'),esc(r.t.cidade||'Cidade não informada'),r.date?dateBR(r.date,false):'-',`<strong class="${r.profit<0?'negative':'positive'}">${money(r.profit)}</strong>`,`<button class="action-btn" data-finance-tender="${esc(r.t.id)}">Ver resultado</button>`])):'<p class="hint">Nenhuma licitação encontrada no período.</p>';
