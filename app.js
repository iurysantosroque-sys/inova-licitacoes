...
async function exportQuotePdf(){
  ...
  const {PDFDocument,rgb,StandardFonts}=window.PDFLib;const base=await fetch('assets/papel-timbrado.pdf').then(r=>r.arrayBuffer());const pdf=await PDFDocument.load(base);let page=pdf.getPages()[0];const font=await pdf.embedFont(StandardFonts.Helvetica);const bold=await pdf.embedFont(StandardFonts.HelveticaBold);const ink=rgb(.12,.12,.12),line=rgb(.72,.72,.72);const x0=60,x1=90,x2=390,x3=455,x4=535,rowFont=8.5,rowGap=11;
  ...
  let y=680;page.drawText('TABELA DE COTAÇÃO',{x:x0,y,size:16,font:bold,color:ink});page.drawText(`${tender.orgao||'Órgão comprador'} - ${tender.cidade||''}`,{x:x0,y:y-24,size:10,font,color:ink});y-=58;
