...
function renderControlDeclarationFiles(area,files){
  area.innerHTML=`<div class="document-control-generated-head"><strong>Declarações</strong><span>${files.length} arquivos prontos</span></div><div class="document-control-generated-list">${files.map(file=>`<a class="document-control-generated-file" href="${file.url}" download="${esc(file.name)}"><img src="assets/controle-documentos.png" alt=""><span>${esc(file.name)}</span><b>⇩</b></a>`).join('')}</div><button type="button" class="document-control-subfolders-back" data-control-subfolders-back>← Voltar às pastas</button>`;
}
